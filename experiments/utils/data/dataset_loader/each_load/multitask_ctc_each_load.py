...
class DatasetBase(object):
    ...
    def __next__(self, batch_size=None):
        ...
        inputs = np.zeros(
            ...
            dtype=np.float32)
        labels_main = np.array([[self.padded_value] * max_seq_len_main] * len(data_indices))
        labels_sub = np.array([[self.padded_value] * max_seq_len_sub] * len(data_indices))
        ...
        for i_batch in range(len(data_indices)):
            ...
            labels_main[i_batch, :len(label_main_list[i_batch])] = label_main_list[i_batch]
            labels_sub[i_batch, :len(label_sub_list[i_batch])] = label_sub_list[i_batch]
        ...
        if self.num_gpu > 1:
            ...
            inputs_seq_len = np.array_split(inputs_seq_len, self.num_gpu, axis=0)
        ...
        return (inputs, labels_main, labels_sub, inputs_seq_len, input_names), self.is_new_epoch
