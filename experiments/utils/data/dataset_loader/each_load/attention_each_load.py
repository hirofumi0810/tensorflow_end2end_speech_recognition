...
class DatasetBase(Base):
    ...
    def __next__(self, batch_size=None):
        ...
        inputs = np.zeros(
            ...
            dtype=np.float32)
        labels = np.array([[self.padded_value] * max_seq_len] * len(data_indices))
        ...
        if self.num_gpu > 1:
            ...
            labels_seq_len = np.array_split(labels_seq_len, self.num_gpu, axis=0)
        ...
        return (inputs, labels, inputs_seq_len, labels_seq_len, input_names), self.is_new_epoch
