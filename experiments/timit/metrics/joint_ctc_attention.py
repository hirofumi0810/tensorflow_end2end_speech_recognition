...
def do_eval_per(session, decode_op, per_op, model, dataset, label_type,
    ...
    train_phone2idx_map_file_path = '../metrics/mapping_files/attention/' + \
        train_label_type + '.txt'
    eval_phone2idx_map_file_path = '../metrics/mapping_files/attention/' + \
        eval_label_type + '.txt'
    ...
    for data, is_new_epoch in wrap_generator(dataset, progressbar, total=total_step):
        ...
        labels_true_st = list2sparsetensor(
            att_labels_true_mapped, padded_value=dataset.att_padded_value)
        labels_pred_st = list2sparsetensor(
            att_labels_pred_mapped, padded_value=dataset.att_padded_value)
