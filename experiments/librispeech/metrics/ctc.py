...
def do_eval_cer(session, decode_op, network, dataset, label_type,
    ...
    cer_mean = 0
    for data, next_epoch_flag in wrap_generator(dataset(batch_size), progressbar, total=int(dataset.data_num / batch_size) + 1):
...
def do_eval_wer(session, decode_op, network, dataset, train_data_size, is_test,
                eval_batch_size=None, progressbar=False, is_multitask=False):
