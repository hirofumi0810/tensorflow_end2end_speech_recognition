...
class Dataset(DatasetBase):
    ...
    def __init__(self, data_type, train_data_size, label_type_main,
                 label_type_sub, batch_size, num_stack=1, num_skip=1,
        ...
        if data_type in ['test_clean', 'test_other'] and label_type_main == 'word':
            self.is_test = True
        else:
            self.is_test = False
        ...
        if is_gpu:
            ...
            if data_type == 'train_all':
                ...
                label_main_path_list = [
                    join('/data/inaguma/librispeech/labels/ctc/train_clean100',
                         label_type_main, 'train_clean100'),
                    join('/data/inaguma/librispeech/labels/ctc/train_clean360',
                         label_type_main, 'train_clean360'),
                    join('/data/inaguma/librispeech/labels/ctc/train_other500',
                         label_type_main, 'train_other500')]
                label_sub_path_list = [
                    join('/data/inaguma/librispeech/labels/ctc/train_clean100',
                         label_type_sub, 'train_clean100'),
                    join('/data/inaguma/librispeech/labels/ctc/train_clean360',
                         label_type_sub, 'train_clean360'),
                    join('/data/inaguma/librispeech/labels/ctc/train_other500',
                         label_type_sub, 'train_other500')]
            else:
                input_path_list = [
        ...
        for input_name, frame_num in frame_num_tuple_sorted:
            ...
            for input_path in input_path_list:
                if isfile(join(input_path, speaker_name, input_name + '.npy')):
                    ...
                    break
            for label_main_path, label_sub_path in zip(label_main_path_list, label_sub_path_list):
                if isfile(join(label_main_path, speaker_name, input_name + '.npy')):
        ...
        assert len(self.input_paths) == len(self.label_main_paths), "Inputs and labels must have the same number of files (inputs: {0}, labels: {1}).".format(
            len(self.input_paths), len(self.label_main_paths))
        assert len(self.input_paths) == len(self.label_sub_paths), "Inputs and labels must have the same number of files (inputs: {0}, labels: {1}).".format(
            len(self.input_paths), len(self.label_sub_paths))
