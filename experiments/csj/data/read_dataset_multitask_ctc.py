...
class DataSet(object):
    ...
    def next_cluster(self):
        ...
        self.input_list = []
        self.label_main_list, self.label_second_list = [], []
    ...
    def next_batch(self, batch_size=None, session=None):
        ...
        if self.is_sorted:
            ...
            labels_main = np.array([[-1] * max_seq_len_main]  # Padding by -1
        ...
        if self.num_gpu > 1:
            ...
            labels_main = tf.split(labels_main, self.num_gpu, axis=0)
            labels_second = tf.split(labels_second, self.num_gpu, axis=0)
            inputs_seq_len = tf.split(inputs_seq_len, self.num_gpu, axis=0)
            input_names = tf.split(input_names, self.num_gpu, axis=0)

            labels_main_st, labels_second_st = [], []
            for i_gpu in range(self.num_gpu):
                labels_main_st.append(list2sparsetensor(
                    session.run(labels_main[i_gpu])))
                labels_second_st.append(list2sparsetensor(
                    session.run(labels_second[i_gpu])))
        else:
            labels_main_st = list2sparsetensor(labels_main)
            labels_second_st = list2sparsetensor(labels_second)

        return (inputs, labels_main_st, labels_second_st, inputs_seq_len,
                input_names)
