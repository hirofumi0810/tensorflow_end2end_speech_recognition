...
class TestAttention(unittest.TestCase):
    ...
    def check_training(self, model_type, label_type):
        ...
        with tf.Graph().as_default():
            ...
            network = blstm_attention.BLSTMAttetion(batch_size=1,
                                                    input_size=inputs[0].shape[1],
                                                    encoder_num_cell=256,
                                                    encoder_num_layers=2,
                                                    attention_dim=128,
                                                    decoder_num_cell=256,
                                                    decoder_num_layers=1,
                                                    output_size=26 + 2,  # + <SOS> & <EOS>
                                                    sos_index=output_size + 1,
                                                    eos_index=output_size + 2,
                                                    parameter_init=0.1,
                                                    clip_grad=5.0,
                                                    clip_activation=50,
