...
MODEL_SAVE_PATH="/speech7/takashi01_nb/inaguma/models/tensorflow/librispeech"

# Select GPU
if [ $# -lt 2 ]; then
  echo "Error: set GPU number & config path." 1>&2
  echo "Usage: ./run_student_cnn_xe.sh path_to_config_file gpu_index1 gpu_index2... (arbitrary number)" 1>&2
  exit 1
fi
...
# CUDA_VISIBLE_DEVICES=$gpu_index nohup $PYTHON train_student_cnn_xe.py \
#   $config_path $MODEL_SAVE_PATH $gpu_index > log/$filename".log" &

# Standard output version
CUDA_VISIBLE_DEVICES=$gpu_index $PYTHON train_student_cnn_xe.py \
